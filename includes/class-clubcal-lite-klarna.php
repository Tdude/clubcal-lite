...
final class ClubCal_Lite_Klarna {
	public const AJAX_CREATE_ORDER = 'clubcal_klarna_create';
	public const REST_CALLBACK = 'clubcal-lite/v1/klarna-callback';
	public const REST_CONFIRMATION = 'clubcal-lite/v1/klarna-confirmation';
	...
	public function register(): void {
		...
		add_shortcode('clubcal_klarna_confirmation', [$this, 'shortcode_confirmation']);
	}
	...
	public function register_rest_routes(): void {
		register_rest_route('clubcal-lite/v1', '/klarna-callback', [
			'methods'             => 'POST',
			'callback'            => [$this, 'handle_callback'],
			'permission_callback' => '__return_true',
		]);

		register_rest_route('clubcal-lite/v1', '/klarna-confirmation/(?P<booking_id>\d+)', [
			'methods'             => 'GET',
	...
	public function create_checkout_order(int $booking_id, float $amount, string $description): array {
		if (!$this->is_configured()) {
			return ['error' => __('Klarna not configured.', 'clubcal-lite')];
		}

		// Get booking details
		$event_id = (int) get_post_meta($booking_id, '_clubcal_booking_event_id', true);
		$email = get_post_meta($booking_id, '_clubcal_booking_email', true);
		$name = get_post_meta($booking_id, '_clubcal_booking_name', true);
		...
		$order_data = [
			...
			'merchant_urls'     => [
				...
				'confirmation' => rest_url('clubcal-lite/v1/klarna-confirmation/' . $booking_id) . '?klarna_order_id={checkout.order.id}',
		...
		if (empty($result['order_id']) || empty($result['html_snippet'])) {
			return ['error' => __('Invalid Klarna response.', 'clubcal-lite')];
		}

		// Store Klarna order ID
		update_post_meta($booking_id, '_clubcal_klarna_order_id', $result['order_id']);
		update_post_meta($booking_id, '_clubcal_klarna_status', 'checkout_incomplete');
	...
	public function ajax_create_order(): void {
		if (!check_ajax_referer('clubcal_klarna_create', '_ajax_nonce', false)) {
			wp_send_json_error(__('Security check failed.', 'clubcal-lite'), 403);
		}

		$booking_id = isset($_POST['booking_id']) ? absint($_POST['booking_id']) : 0;
		if (!$booking_id) {
			wp_send_json_error(__('Invalid booking.', 'clubcal-lite'), 400);
		}
		...
		if ($amount <= 0) {
			wp_send_json_error(__('Invalid amount.', 'clubcal-lite'), 400);
		}
	...
	public function handle_callback(\WP_REST_Request $request): \WP_REST_Response {
		...
		$klarna_order_id = get_post_meta($booking_id, '_clubcal_klarna_order_id', true);
		if (!$klarna_order_id) {
		...
		$status = $order['status'] ?? '';
		update_post_meta($booking_id, '_clubcal_klarna_status', $status);
	...
	public function shortcode_confirmation(array $atts = []): string {
		...
		$confirmation_code = get_post_meta($booking_id, '_clubcal_booking_confirmation_code', true);
		$event_id = (int) get_post_meta($booking_id, '_clubcal_booking_event_id', true);
		$event_title = get_the_title($event_id);
		$event_start = get_post_meta($event_id, '_clubcal_start', true);

		$html = '<div class="clubcal-lite-confirmation">';
		$html .= '<h2>' . esc_html__('Booking Confirmed!', 'clubcal-lite') . ' ✓</h2>';
		$html .= '<p><strong>' . esc_html($event_title) . '</strong></p>';
		if ($event_start) {
			$html .= '<p>' . esc_html(wp_date('l j F Y, H:i', strtotime($event_start))) . '</p>';
		}
		$html .= '<p>' . esc_html__('Confirmation Code:', 'clubcal-lite') . ' <code>' . esc_html($confirmation_code) . '</code></p>';
		$html .= '<p>' . esc_html__('A confirmation email has been sent to you.', 'clubcal-lite') . '</p>';
		$html .= '</div>';
	...
	public static function get_checkout_snippet(int $booking_id): string {
		...
		if (!$klarna->is_configured()) {
			return '<p>' . esc_html__('Payment not available.', 'clubcal-lite') . '</p>';
		}

		// Check if we already have an order
		$existing_order_id = get_post_meta($booking_id, '_clubcal_klarna_order_id', true);
		...
		return '<p>' . esc_html__('Could not load payment form.', 'clubcal-lite') . '</p>';
	}
}
