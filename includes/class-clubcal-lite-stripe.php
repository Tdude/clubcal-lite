...
class ClubCal_Lite_Stripe {
    ...
    public function register(): void {
        // AJAX endpoints
        add_action( 'wp_ajax_clubcal_create_stripe_session', array( $this, 'ajax_create_checkout_session' ) );
        add_action( 'wp_ajax_nopriv_clubcal_create_stripe_session', array( $this, 'ajax_create_checkout_session' ) );
        
        add_action( 'wp_ajax_clubcal_check_stripe_payment', array( $this, 'ajax_check_payment_status' ) );
        add_action( 'wp_ajax_nopriv_clubcal_check_stripe_payment', array( $this, 'ajax_check_payment_status' ) );
    ...
    public function register_rest_routes(): void {
        register_rest_route( 'clubcal-lite/v1', '/stripe-webhook', array(
            'methods'             => 'POST',
    ...
    public static function get_settings(): array {
        $payment_settings = get_option( ClubCal_Lite_Payment::OPTION_KEY, array() );
        return array(
    ...
    private static function api_request( string $endpoint, array $data = array(), string $method = 'POST' ) {
        ...
        if ( empty( $settings['secret_key'] ) ) {
            return new WP_Error( 'no_api_key', __( 'Stripe API key not configured', 'clubcal-lite' ) );
        }
        ...
        if ( $code >= 400 ) {
            $error_message = isset( $body['error']['message'] ) ? $body['error']['message'] : __( 'Stripe API error', 'clubcal-lite' );
    ...
    public function create_checkout_session( int $booking_id, float $amount = 0, string $description = '' ): array {
        ...
        $event_id = get_post_meta( $booking_id, '_clubcal_booking_event_id', true );
        $event    = get_post( $event_id );
        $customer_name  = get_post_meta( $booking_id, '_clubcal_booking_name', true );
        $customer_email = get_post_meta( $booking_id, '_clubcal_booking_email', true );
        ...
        if ( $amount <= 0 ) {
            $price_str = get_post_meta( $event_id, '_clubcal_price', true );
            $amount = (float) preg_replace( '/[^0-9.]/', '', $price_str );
        }

        if ( $amount <= 0 ) {
            return array( 'success' => false, 'error' => __( 'No payment amount specified', 'clubcal-lite' ) );
        }

        // Use provided description or event title
        $product_name = ! empty( $description ) ? $description : ( $event ? $event->post_title : __( 'Event Booking', 'clubcal-lite' ) );
    ...
    public function ajax_create_checkout_session(): void {
        check_ajax_referer( 'clubcal_booking', 'nonce' );

        $booking_id = isset( $_POST['booking_id'] ) ? intval( $_POST['booking_id'] ) : 0;
        $return_url = isset( $_POST['return_url'] ) ? esc_url_raw( $_POST['return_url'] ) : '';

        if ( ! $booking_id ) {
            wp_send_json_error( array( 'message' => __( 'Missing booking ID', 'clubcal-lite' ) ) );
        }

        $result = $this->create_checkout_session( $booking_id );

        if ( empty( $result['success'] ) ) {
            wp_send_json_error( array( 'message' => $result['error'] ?? __( 'Failed to create checkout session', 'clubcal-lite' ) ) );
        }
    ...
    public function ajax_check_payment_status(): void {
        ...
        if ( empty( $session_id ) ) {
            wp_send_json_error( array( 'message' => __( 'No Stripe session found', 'clubcal-lite' ) ) );
        }
        ...
        $booking_status = get_post_meta( $booking_id, '_clubcal_booking_status', true );
    ...
    private static function confirm_booking_payment( int $booking_id, array $session ): void {
        ...
        if ( class_exists( 'ClubCal_Lite_Payment' ) ) {
            ClubCal_Lite_Payment::log_payment( array(
                'booking_id' => $booking_id,
                'event_id'   => $event_id,
                'method'     => 'stripe',
                'status'     => 'completed',
        ...
        if ( class_exists( 'ClubCal_Lite_Mailchimp' ) ) {
            ClubCal_Lite_Mailchimp::send_booking_confirmation( $booking_id );
        }

        // Fire action for extensibility
        do_action( 'clubcal_booking_payment_confirmed', $booking_id, 'stripe', $session );
    }
}
