...
final class ClubCal_Lite_Swish {
	public const AJAX_CREATE_PAYMENT = 'clubcal_swish_create';
	public const AJAX_CHECK_PAYMENT = 'clubcal_swish_check';
	public const REST_CALLBACK = 'clubcal-lite/v1/swish-callback';
	...
	public function register_rest_routes(): void {
		register_rest_route('clubcal-lite/v1', '/swish-callback', [
			'methods'             => 'POST',
	...
	public function create_payment_request(int $booking_id, float $amount, string $message = ''): array {
		...
		set_transient('clubcal_swish_' . $payment_reference, $payment_data, 15 * MINUTE_IN_SECONDS);

		// Also save reference to booking meta
		update_post_meta($booking_id, '_clubcal_swish_reference', $payment_reference);
		update_post_meta($booking_id, '_clubcal_swish_status', 'CREATED');
		...
		return [
			...
			'check_url'         => admin_url('admin-ajax.php') . '?' . http_build_query([
				...
				'_ajax_nonce' => wp_create_nonce('clubcal_swish_check_' . $payment_reference),
			]),
		];
	}
	...
	public function ajax_create_payment(): void {
		if (!check_ajax_referer('clubcal_swish_create', '_ajax_nonce', false)) {
		...
		$message = sprintf(__('Booking %s', 'clubcal-lite'), substr($event_title, 0, 30));
	...
	public function ajax_check_payment(): void {
		...
		if (!$reference || !check_ajax_referer('clubcal_swish_check_' . $reference, '_ajax_nonce', false)) {
			wp_send_json_error(__('Invalid request.', 'clubcal-lite'), 400);
		}

		// Check transient first
		$payment_data = get_transient('clubcal_swish_' . $reference);
		
		if (!$payment_data) {
			// Fallback to meta lookup
			$args = [
				'post_type'      => ClubCal_Lite_Booking::POST_TYPE,
				'posts_per_page' => 1,
				'meta_query'     => [
					[
						'key'   => '_clubcal_swish_reference',
						'value' => $reference,
					],
				],
			];
			...
			if ($query->have_posts()) {
				...
				$status = get_post_meta($booking_id, '_clubcal_swish_status', true);
			} else {
				wp_send_json_error(__('Payment not found.', 'clubcal-lite'), 404);
				return;
			}
		} else {
	...
	public function handle_callback(\WP_REST_Request $request): \WP_REST_Response {
		...
		// Update status
		update_post_meta($booking_id, '_clubcal_swish_status', $status);

		// Update transient
		$payment_data = get_transient('clubcal_swish_' . $reference);
		if ($payment_data) {
			$payment_data['status'] = $status;
			set_transient('clubcal_swish_' . $reference, $payment_data, 15 * MINUTE_IN_SECONDS);
		}
	...
	public function simulate_payment_complete(string $reference): bool {
		...
		// Update status
		update_post_meta($booking_id, '_clubcal_swish_status', 'PAID');
		update_post_meta($booking_id, '_clubcal_booking_status', 'confirmed');
