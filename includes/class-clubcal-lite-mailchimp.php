...
final class ClubCal_Lite_Mailchimp {
	...
	public function register(): void {
		...
		add_action('clubcal_lite_booking_created', [$this, 'on_booking_created'], 10, 2);
		add_action('clubcal_lite_payment_completed', [$this, 'on_payment_completed'], 10, 2);
	}
	...
	public function send_booking_confirmation(int $booking_id, array $booking_data): bool {
		...
		$confirmation_code = $booking_data['confirmation_code'] ?? get_post_meta($booking_id, '_clubcal_booking_confirmation_code', true);

		// Format date
		$date_formatted = $event_start ? wp_date('l j F Y, H:i', strtotime($event_start)) : '';

		$subject = sprintf(__('Booking Confirmed: %s', 'clubcal-lite'), $event_title);

		// Build email body
		$body = sprintf(__('Hi %s,', 'clubcal-lite'), $name) . "\n\n";
		$body .= __('Your booking has been confirmed!', 'clubcal-lite') . "\n\n";
		$body .= "---\n\n";
		$body .= sprintf(__('Event: %s', 'clubcal-lite'), $event_title) . "\n";
		if ($date_formatted) {
			$body .= sprintf(__('Date: %s', 'clubcal-lite'), $date_formatted) . "\n";
		}
		if ($event_location) {
			$body .= sprintf(__('Location: %s', 'clubcal-lite'), $event_location) . "\n";
		}
		$body .= sprintf(__('Confirmation Code: %s', 'clubcal-lite'), $confirmation_code) . "\n\n";
		$body .= "---\n\n";
		$body .= __('See you there!', 'clubcal-lite') . "\n";
